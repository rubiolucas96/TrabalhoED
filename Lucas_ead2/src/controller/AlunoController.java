...
public class AlunoController implements ActionListener {
	...
	private int getNumAlunosPorCurso(int cursoId, Lista lista) throws Exception {
	    int count = 0;
	    int tamanho = lista.size();
	    for (int i = 0; i < tamanho; i++) {
	        Aluno aluno = (Aluno) lista.get(i);
	        if (aluno.curso == cursoId) {
	            count++;
	        }
	    }
	    return count;
	...
	private boolean verificarVagasCurso(int cursoId, Lista lista) {
	    int numAlunosCadastrados = 0;
		try {
			numAlunosCadastrados = getNumAlunosPorCurso(cursoId, lista);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	    return numAlunosCadastrados < 4;
	}
	...
	private void inserir(Lista lista) throws Exception {
		...
		if (!verificarVagasCurso(aluno.curso, lista)) {
	        JOptionPane.showMessageDialog(null, "Não há vagas disponíveis neste curso.");
	        return; 
	    }
		else {
